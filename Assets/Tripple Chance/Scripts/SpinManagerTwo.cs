...
public class SpinManagerTwo : MonoBehaviour
{
    ...
    public float[] doubleSpinAngularVelocities;
    ...
    void Start()
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            spinWheelNumbers.Add(doubleSpinAngularVelocities[i]);
        }
    ...
    private void FixedUpdate()
    {
        ...
        if (spinWheel)
        {
            ...
            int digit = (int)char.GetNumericValue(Datas.instance.winDigits[1]);
            SpinWheelTwo(doubleSpinAngularVelocities[digit], this.GetComponent<SpinManagerTwo>());
    ...
    public void SpinWheelTwo(float angularDrag, SpinManagerTwo SpinWheel)
    {
       // Debug.Log("Angular Drag is " + angularDrag);
        if (Torque == 0f)
            return;
    ...
    public void CalculateWinNUmber()
    {
        for (int i = 0; i < 10; i++)
        {
            if (i == System.Array.IndexOf(doubleSpinAngularVelocities, Body.angularDrag))
            {
                doublewinNumber = spinWheelNumbers.IndexOf(Body.angularDrag);
                GameManager.Instance.doubleResult = doublewinNumber;
            }
        }
